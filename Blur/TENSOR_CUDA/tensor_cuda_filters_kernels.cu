...
Kernel TENSOR
*/
__global__ void Tensor(uint8_t* const blurredImage, const uint8_t* const rawImage, int width, int height, int channels, const half* filter, int filterWidth, int numFilters)
{
...
__global__ void Cuda(uint8_t* const blurredImage, const uint8_t* const rawImage, int width, int height, int channels, const half* filter, int filterWidth, int numFilters, int initialBlock)
{
        // Identificadores de threads
        int pos = (blockIdx.x + initialBlock) * blockDim.x + threadIdx.x;
...
{        
        ...
        if (blockIdx.x < blockTensor) {
                ...
                Tensor(blurredImage, rawImage, width, height, channels, filter, filterWidth, numFilters);
        } else {
                // Implementacion CUDA
                Cuda(blurredImage, rawImage, width, height, channels, filter, filterWidth, numFilters, blockTensor);
        }
}
*/
...
int main(int argc, char** argv)
{
	...
	printf("Width:%d, Height:%d Size(in Bytes):%lu\n", width, height, (long unsigned int) width*height*bpp*channels);
        ...
        int gridDim((width * height) / ((threadsPerBlock) / channels) + 1);

        // initial balancer value
        float balancer = 0.5f;
        int blocksTensor = balancer * gridDim;
        dim3 gridDimTensor(blocksTensor);
        ...
        Tensor<<<gridDimTensor, blockDim, sharedMemorySize>>>(d_blurredImage, d_originalImage, width, height, channels, d_filter, filterWidth, numFilters);
        Cuda<<<gridDimCuda, blockDim, sharedMemorySize>>>(d_blurredImage, d_originalImage, width, height, channels, d_filter, filterWidth, numFilters, blocksTensor);
