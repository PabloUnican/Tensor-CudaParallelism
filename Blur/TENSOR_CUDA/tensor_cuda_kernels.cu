...
*/ 
__global__ void GaussianBlurOnTENSOR(uint8_t* const blurredImage, const uint8_t* const rawImage, int width, int height, int channels, const half* filter, int filterWidth)
{        
...
Estructura unidimensional de threads (x para posicion y canal)
*/ 
__global__ void GaussianBlurOnCUDA(uint8_t* const blurredImage, const uint8_t* const rawImage, int width, int height, int channels, const half* filter, int filterWidth, int initialBlock) {        
        // Identificadores de threads
        int temp = (blockIdx.x + initialBlock) * blockDim.x + threadIdx.x;
...
int main(int argc, char** argv)
{
	...
	printf("Width:%d, Height:%d Size(in Bytes):%lu\n", width, height, (long unsigned int) width*height*bpp*channels);
        ...
        int numGrids = (width * height) / ((threadsPerBlock) / channels) + 1;
        float balancer = 0.5;
        int blocksTensor = balancer * numGrids;
        dim3 gridDimCuda(numGrids - blocksTensor);
        ...
        GaussianBlurOnTENSOR<<<gridDimTensor, blockDim, sharedMemorySize>>>(d_blurredImage, d_originalImage, width, height, channels, d_filter, filterWidth);
        GaussianBlurOnCUDA<<<gridDimCuda, blockDim, sharedMemorySize>>>(d_blurredImage, d_originalImage, width, height, channels, d_filter, filterWidth, blocksTensor);
