...
#define N 16384 // Size of the matrices

__global__ void matrixMulKernel(half* d_A, half* d_B, float* d_C, int width, float coeficient) {
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    half sum = 0;

    int numTensor = coeficient * width;
    if (row >= numTensor) {
...
void matrixMul(half* h_A, half* h_B, float* h_C, int width) {
    ...
    matrixMulKernel<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B, d_C, width, 1.0);
